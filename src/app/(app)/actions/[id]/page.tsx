...
export default async function ActionDetailPage({
  ...
  const deleteBound = deleteActionAction.bind(null, id);
  const canDelete = profile.role === "admin" && action.status === "closed";
    ...
    <div>
      ...
      {canDelete && (
        <div className="mt-8 max-w-lg">
          <DeleteActionButton title={action.title} action={deleteBound} />
        </div>
      )}
    </div>
  );
